...
class Element:
	def __init__(self , title="An element of a generic message." , subtitle=None , image_url=None , buttons=None):
		...
		"""
		assert isinstance(title , str) , f"type of param title must be str , not {type(title)}"
	...
	def set_subtitle(self , subtitle):
		assert isinstance(subtitle , str) , f"type of param subtitle must be str , not {type(subtitle)}"
		self.__subtitle = subtitle

	def set_image_url(self , image_url):
		assert isinstance(image_url , str) , f"type of param image_url must be str , not {type(image_url)}"

		self.__image_url = image_url

	def add_button(self , button):
		assert isinstance(button , dict) , f"type of param button must be list , not {type(button)}"
...
class Button:
	def __init__(self , button_type=POSTBACK , title="Button"):
		...
		"""
		assert button_type in [POSTBACK , WEB_URL] , "param type must be POSTBACK or WEB_URL"
		assert isinstance(title , str) , f"type of param title must be str , not {type(title)}"
		assert title != "" , "param title must be non empty"
		...
		if self.__type == POSTBACK :
			self.__payload = "<DEVELOPER_DEFINED_PAYLOAD>"
		elif self.__type == WEB_URL:
	...
	def set_payload(self , payload):
		assert self.__type == POSTBACK  , "param payload is only supported on postback buttons"
		assert isinstance(payload , str) , f"type of param payload must be str , not {type(payload)}"

		self.__payload = payload

	def set_url(self , url):
		assert self.__type == WEB_URL , "param url is only supported on web_url buttons"
...
class QuickReply:
	def __init__(self , title="Quick reply" , payload="<DEVELOPER_DEFINED_PAYLOAD>" , image_url=None):
		...
		if len(title) > 20:
			print("WARNING : max characters for param title is 20 , your title won't show entirely")
...
class PersistentMenu:
	def __init__(self , default_locale_menu):
		...
		"""
		assert isinstance(default_locale_menu , list) , f"type of param default_locale_menu must be a list of Button objects"
		assert len(default_locale_menu) > 0 , "param default_locale_menu must contains at least one Button object"
	...
	def add_locale(self , language_code , menu):
		...
		"""
		assert isinstance(language_code , str) , f"type of param language_code must be a string , not {type(language_code)}"
		assert isinstance(menu , list) , f"type of param menu must be a list of Button object"
